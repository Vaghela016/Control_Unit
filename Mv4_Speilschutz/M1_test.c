...
void adc_init(void) {
    ...
    // Assuming a 16MHz clock: 16,000,000 / 128 = 125kHz.
...
ISR(TIMER2_OVF_vect) {
    ...
    for (uint8_t i = 0; i < 4; i++) {
        if (M_Active[i] > 0) {
            ...
            if (S_ticks >= 61) { // 61 ticks ~ 2 seconds at ~61 Hz
               ...
               if (M_FWD[i] > 0) {
                ...
                S_ticks = 61; // Cap switch tick counter so it doesn't overflow past 255
            }
        }
        else {
            S_ticks = 0; // Reset switch tick counter if motor is inactive
        }
    }
