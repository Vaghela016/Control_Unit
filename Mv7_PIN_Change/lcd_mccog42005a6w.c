...
#include "i2cmaster.h" // Include Peter Fleury's library
...
/* ---- Unified TWI master (using i2cmaster.h) ---- */

static bool twi_tx(uint8_t addr7, const uint8_t *bytes, uint8_t len) {
    if (len == 0) return true;

    // i2c_start expects the 8-bit address (shifted 7-bit + Write bit)
    if (i2c_start((addr7 << 1) | I2C_WRITE) != 0) {
        i2c_stop();
        return false;
    }

    for (uint8_t i = 0; i < len; i++) {
        if (i2c_write(bytes[i]) != 0) {
            i2c_stop();
            return false;
        }
    }
...
void lcdI2CInit(uint32_t scl_hz) {
    // Rely on Peter Fleury's i2c_init() called in main()
    // We can leave this empty to prevent overriding TWBR settings.
    (void)scl_hz; 
}

bool lcdI2CIsInitialized(void) {
    // Assume initialized since i2c_init() is called at the start of main()
    return true;
}
